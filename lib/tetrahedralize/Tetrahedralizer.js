...
export class Tetrahedralizer {
     ...
     * Uses odd-even rule: count ray intersections, odd = inside
    ...
    isInside(point, minDist = 0) {
        ...
        // Test multiple directions and vote
        for (const dir of CAST_DIRECTIONS) {
            ray.origin.copy(point);
            ray.direction.copy(dir);

            // Use BVH raycast with DoubleSide to hit both front and back faces
            // This is critical for inside/outside testing
            ...
            if (minDist > 0 && hits.length > 0 && hits[0].distance < minDist) {
                return false;
            }
        ...
        // Majority vote (at least 4 out of 6 directions agree)
    ...
    createTetIds(verts) {
        ...
        for (let i = 0; i < numTets; i++) {
            ...
            // Quality check (use absolute value since volume can be negative due to winding)
            const quality = Math.abs(tetQuality(p0, p1, p2, p3));
            if (quality < this.minQuality) {
                numBad++;
                continue;
            }

            // Inside check - verify centroid is inside the original mesh
            const centroid = new THREE.Vector3()
                .add(p0).add(p1).add(p2).add(p3)
                .multiplyScalar(0.25);

            if (this.bvh && !this.isInside(centroid, 0)) {
                numOutside++;
                continue;
            }
        ...
        this.log(`Filtering: ${numDeleted} deleted/super, ${numBad} bad quality, ${numOutside} outside`);
    ...
    tetrahedralize(geometry) {
        ...
        if (this.resolution > 0) {
            ...
            this.log(`Interior sampling: tested ${interiorTested}, added ${interiorAdded}`);
        }
