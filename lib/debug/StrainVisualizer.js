...
export class StrainVisualizer {
    ...
    createMaterial(geometry) {
        ...
        material.colorNode = Fn(() => {
            ...
            // Map to color
            const normalizedStrain = clamp(avgStrain.div(this.maxStrain), -1, 1);

            // Negative = compression (blue), positive = tension (red)
            const color = mix(
                mix(compressionCol, neutralCol, normalizedStrain.add(1).mul(0.5)),
                mix(neutralCol, tensionCol, normalizedStrain.mul(0.5).add(0.5)),
                normalizedStrain.add(1).mul(0.5)
            );
