...
export class TetVisualizer {
     ...
     * @param {number} [options.opacity=0.3] - Face opacity
    ...
    _createFaces(vertices, tets, colorByQuality) {
        ...
        for (const tet of tets) {
            ...
            if (colorByQuality) {
                const quality = tetQuality(v[0], v[1], v[2], v[3]);
                // Map quality to color (red = bad, green = good)
                color = new THREE.Color().setHSL(quality * 0.33, 1, 0.5);
            }
    ...
    static getStatistics(tetVerts, tetIds) {
        ...
        for (let i = 0; i < tetIds.length; i += 4) {
            ...
            const quality = tetQuality(v0, v1, v2, v3);
