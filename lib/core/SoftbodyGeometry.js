...
export class SoftbodyGeometry {
    ...
    /**
     * Bakes the geometry (creates buffers and updates material)
     */
    async bake() {
        // Recreate material to reference new physics buffers
        // The positionNode captures buffer references at creation time,
        // so we must recreate it when buffers change during rebake
        const materialClass = this.material.constructor;
        const oldColor = this.material.color?.clone();
        const oldRoughness = this.material.roughness;
        const oldMetalness = this.material.metalness;

        this.material.dispose();
        this.createMaterial(materialClass);

        // Restore material properties
        if (oldColor) this.material.color = oldColor;
        if (oldRoughness !== undefined) this.material.roughness = oldRoughness;
        if (oldMetalness !== undefined) this.material.metalness = oldMetalness;
    ...
    createMaterial(materialClass) {
        ...
        material.positionNode = Fn(() => {
            ...
            // Rotate normal by tet quaternion and store in varying for normalNode
        ...
        // Set normalNode to use the rotated normal from the varying
        // This is required in three.js r182+ for custom normals to work
