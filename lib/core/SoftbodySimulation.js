...
export class SoftbodySimulation {
    ...
    async _compileKernels() {
        ...
        this.kernels.solveElemPass = Fn(() => {
            ...
            If(instanceIndex.lessThan(this.uniforms.tetCount), () => {
                ...
                If(size.greaterThanEqual(0.0001), () => {
                    ...
                    // Calculate current volume using signed volume formula: V = dot(cross(a, b), c) / 6
                    const edge01 = ref1.sub(ref0).toVar();
                    const edge02 = ref2.sub(ref0).toVar();
                    const edge03 = ref3.sub(ref0).toVar();
                    const crossEdge = cross(edge01, edge02).toVar();
                    const currentVolume = dot(crossEdge, edge03).div(6.0).toVar();

                    // Get rest volume
                    const restVolume = tetBuffer.get(instanceIndex, 'restVolume');

                    // Calculate surface area (sum of 4 triangle faces)
                    // Triangle area = |cross(edge1, edge2)| / 2
                    const edge12 = ref2.sub(ref1).toVar();
                    const edge13 = ref3.sub(ref1).toVar();
                    const edge23 = ref3.sub(ref2).toVar();

                    // Face 0-1-2
                    const area012 = cross(edge01, edge02).length().mul(0.5);
                    // Face 0-1-3
                    const area013 = cross(edge01, edge03).length().mul(0.5);
                    // Face 0-2-3
                    const area023 = cross(edge02, edge03).length().mul(0.5);
                    // Face 1-2-3
                    const area123 = cross(edge12, edge13).length().mul(0.5);

                    const surfaceArea = area012.add(area013).add(area023).add(area123).toVar();

                    // Calculate delta volume with inversion prevention
                    // Only restore volume if we're not more than 2x larger (prevents snap-back inversion)
                    const deltaVolume = select(
                        currentVolume.lessThan(restVolume.mul(2.0)),
                        restVolume.sub(currentVolume),
                        float(0.0)
                    ).toVar();

                    // Calculate dilation distance: dilate each vertex along its normal from centroid
                    const dilationDistance = deltaVolume.div(surfaceArea.max(0.0001)).toVar();

                    // Apply dilation: move each vertex away from centroid
                    const restCentroid = ref0.add(ref1).add(ref2).add(ref3).mul(0.25).toVar();
                    const dir0 = normalize(ref0.sub(restCentroid)).toVar();
                    const dir1 = normalize(ref1.sub(restCentroid)).toVar();
                    const dir2 = normalize(ref2.sub(restCentroid)).toVar();
                    const dir3 = normalize(ref3.sub(restCentroid)).toVar();

                    ref0.addAssign(dir0.mul(dilationDistance));
                    ref1.addAssign(dir1.mul(dilationDistance));
                    ref2.addAssign(dir2.mul(dilationDistance));
                    ref3.addAssign(dir3.mul(dilationDistance));
        ...
        this.renderer.compute(this.kernels.solveElemPass);
        ...
        this.renderer.compute(this.kernels.applyElemPass);
