...
export class SoftbodySimulation {
    ...
    constructor(renderer, config = {}) {
        ...
        // Coupled sub-systems that step inside the fixed-step loop (see update()).
        this.subStepCallbacks = [];
    }

    /**
     * Registers a callback invoked once per fixed physics substep, after vertex
     * integration and while the tet spatial grid is still populated. Used to couple
     * external GPU systems (e.g. clay particles) to the softbody solver.
     * @param {(renderer: THREE.WebGPURenderer, dt: number) => void} callback
     * @returns {() => void} Unregister function
     */
    onSubStep(callback) {
        this.subStepCallbacks.push(callback);
        return () => {
            const i = this.subStepCallbacks.indexOf(callback);
            if (i >= 0) this.subStepCallbacks.splice(i, 1);
        };
    }
        ...
        this.kernels.applyElemPass = Fn(() => {
            ...
            If(instanceIndex.lessThan(this.uniforms.vertexCount), () => {
                ...
                If(size.greaterThanEqual(0.0001), () => {
                    ...
                    const count = vertexBuffer.get(instanceIndex, 'influencerCount').toVar();
                    const ptrEnd = ptrStart.add(count).toVar();
                    const position = vec3().toVar();
                    const weight = float().toVar();
                    const positionSimple = vec3().toVar(); // unweighted fallback
                    ...
                    Loop({ start: ptrStart, end: ptrEnd, type: 'uint', condition: '<' }, ({ i }) => {
                        ...
                        positionSimple.addAssign(restPosition);
                    });
                    // Guard against degenerate bindings (all influencer tets ~zero
                    // volume): volume-weighting would divide by ~0 (NaN). Fall back to
                    // the plain average of the influencer rest positions so the vertex
                    // still follows its tets instead of getting stuck/stretched.
                    const safeCount = float(count).max(1.0);
                    position.assign(select(weight.greaterThan(1e-6), position.div(weight.max(1e-6)), positionSimple.div(safeCount)));
    ...
    async update(deltaTime, elapsed = 0) {
        ...
        while (this.timeSinceLastStep >= timePerStep) {
            ...
            // Coupled sub-systems (e.g. clay particles) step in lockstep here, after
            // vertices are finalized and while the tet spatial grid is still valid.
            for (let i = 0; i < this.subStepCallbacks.length; i++) {
                this.subStepCallbacks[i](this.renderer, timePerStep);
            }
        }
