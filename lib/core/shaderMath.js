/**
 * TSL shader math utilities for GPU compute
 * @module tetrament/core/shaderMath
 */
...
/**
 * MurmurHash3 for 3D integer coordinates
 */
export const murmurHash13 = Fn(([src_immutable]) => {
    const src = uvec3(src_immutable.add(1073741823)).toVar();
    ...
    name: 'murmurHash13',
    type: 'uint',
    inputs: [{ name: 'src', type: 'ivec3' }]
});

/**
 * Converts axis-angle to quaternion
 */
export const rotationToQuaternion = Fn(([axis_immutable, angle_immutable]) => {
 ...
 */
export const rotateByQuat = Fn(([pos_immutable, quat_immutable]) => {
 ...
 */
export const quat_conj = Fn(([q_immutable]) => {
    const q = vec4(q_immutable).toVar();
    return normalize(vec4(q.x.negate(), q.y.negate(), q.z.negate(), q.w));
}).setLayout({
    name: 'quat_conj',
    type: 'vec4',
    inputs: [{ name: 'q', type: 'vec4' }]
});

/**
 * Quaternion multiplication
 */
export const quat_mult = Fn(([q1_immutable, q2_immutable]) => {
...
/**
 * Extracts rotation quaternion from covariance matrix using polar decomposition
 */
export const extractRotation = Fn(([A_immutable, q_immutable, steps = 3]) => {
    const q = vec4(q_immutable).toVar();
    ...
    Loop({ start: int(0), end: steps, name: 'iter' }, ({ iter }) => {
        ...
        const omega = vec3(
            cross(X, A.element(int(0)))
                .add(cross(Y, A.element(int(1))))
                .add(cross(Z, A.element(int(2))))
                .mul(div(1.0,
                    abs(
                        dot(X, A.element(int(0)))
                            .add(dot(Y, A.element(int(1))))
                            .add(dot(Z, A.element(int(2))))
                            .add(0.000000001)
                    )
                ))
        ).toVar();
