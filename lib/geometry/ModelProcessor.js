...
export function processTetGeometry(tetVertsRaw, tetIdsRaw) {
    ...
    // 1-based: min index is 1, max index equals vertices.length
    // 0-based: min index is 0, max index equals vertices.length - 1
    const minIdx = Math.min(...tetIdsRaw);
    const maxIdx = Math.max(...tetIdsRaw);
    const isOneBased = minIdx >= 1 && maxIdx >= vertices.length;
